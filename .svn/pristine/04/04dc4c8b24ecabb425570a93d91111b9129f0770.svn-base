...
public class XCRoundImageViewByXfermode extends ImageView{
    ...
    private static final int DEFAULT_ROUND_BORDER_RADIUS = 23;//默认圆角大小 
    	...
    	try {
    		...
    		if(bmp == null || bmp.isRecycled()){
    			...
    			if(null != drawable){
    				...
    				int width=(getWidth()>0)?getWidth():dwidth;
    				int height=(getHeight()>0)?getHeight():dheight;
    				//Log.v("czm","dwidth="+dwidth+",width="+getWidth());
    				bmp = Bitmap.createBitmap(getWidth(), height,  
    						Config.ARGB_8888); 
    				float scale = 1.0f;
    				//创建画布
    				...
    				{//如果是圆角矩形或椭圆 
    					...
    					scale = Math.max(getWidth() * 1.0f / dwidth, height  
    							* 1.0f / dheight);  
    				}
