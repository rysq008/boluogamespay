...
//        downloador.updateDownloadInfo(null);
    ...
    protected Long doInBackground(String... params) {
        ...
        try {
            //本地文件
            File file = new File(downloador.getDownloadPath() + File.separator + url.substring(url.lastIndexOf("/") + 1)+ ".apk");

            //获取之前下载保存的信息
            downloadInfo = DBManager.getInstance(context).getDownloadInfoByTaskIdAndUrl(taskId, url);
            //从之前结束的位置继续下载
            ...
            while((inputSize = is.read(buffer)) != -1) {
                ...
                if (isCancelled()) {
                    ...
                    //保存下载信息到数据库
                    DBManager.getInstance(context).insertDownloadInfo(downloadInfo);
                    return null;
                }
            }
